...
4. Set your specific configurations for the workflow in the 00_configurations script. 
<br>
...
* **project**: Specifies the name of your project. A folder with this name will be created automatically in the `./data/projects` folder, and all workflow outputs will be stored inside this folder.
...
* **occurrence_thinning_method**: When a species has more than 10,000 occurrences, records are thinned to 10,000 to match the number of pseudoabsences. Options:
  ...
  * `"kmeans_clustering"`: performs k-means clustering in environmental space and selects 10,000 cluster centroids, ensuring the retained occurrences represent the broadest environmental variation.<br>
<br>
...
4.	It detects and removes highly correlated predictors. Highly correlated predictors can have undesirable effects and confuse the interpretation of variable importance.
<br>
<br>
In the future, the following functionalities will be added:
1.	Automatic generation of confidence maps for each habitat suitability map. These will illustrate confidence of each individual prediction across the study extent.
2.	Evaluate spatial autocorrelation in model residuals to detect clustering effects. If autocorrelation is high, the workflow will recommend (but not automatically apply) occurrence thinning.
<br>
