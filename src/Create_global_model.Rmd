...
global.occ.LL<-data.frame(global.occ)[c(99:98,203,193,224)] #extract long and lat, n= 1758
...
wwf_eco<-st_transform(wwf_eco, 4326) %>%
          st_make_valid()
occ_ecoIntersect <- st_intersects(wwf_eco,global.occ.sf) 
...
biasgrid<-raster(here("./data/external/bias_grids/final/trias/plants_1deg_min5.tif"))}

if(speciesgroup=="Amphibians"){
biasgrid<-raster(here("./data/external/bias_grids/final/trias/amphib_1deg_min5.tif"))}

if(speciesgroup=="Birds"){
biasgrid<-raster(here("./data/external/bias_grids/final/trias/birds_1deg_min5.tif"))}

if(speciesgroup=="Mammals"){
biasgrid<-raster(here("./data/external/bias_grids/final/trias/mammals_1deg_min5.tif"))}

if(speciesgroup=="Molluscs"){
biasgrid<-raster(here("./data/external/bias_grids/final/trias/molluscs_1deg_min5.tif"))}

if(speciesgroup=="Reptiles"){
biasgrid<-raster(here("./data/external/bias_grids/final/trias/reptiles_1deg_min5.tif"))}
...
set.seed(728)
global_points<-dismo::randomPoints(biasgrid_sub,numb.global.pseudoabs, st_drop_geometry(global.occ.sf), ext=NULL, extf=1.1, excludep=TRUE, prob=FALSE, cellnumbers=FALSE, tryf=70, warn=2, lonlatCorrection=TRUE) 
...
system.time({
 global_model<-predict_large_raster(eu_climpreds.10,global_stack,type="prob")
})

```
http://127.0.0.1:34839/graphics/plot_zoom_png?width=1422&height=526
...
```{r exportGlobal}
writeRaster(global_model, filename=file.path(rasterOutput,paste("GlobalEnsEU_",taxonkey,"_",species, ".tif",sep="")),overwrite=TRUE) 
```
