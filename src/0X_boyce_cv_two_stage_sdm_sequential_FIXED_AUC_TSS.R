...
##                                                                            ##
##  Two-stage SDM cross-validated evaluation                                 ##
##  Metrics: Boyce index, AUC, and max TSS                                   ##
##                                                                            ##
...
##    • Boyce / AUC / max-TSS (train & test) per fold for:                  ##
...
# ---- Results directory -------------------------------------------------------
results_dir <- pr("data/projects", project, "Results_CV_Boyce_AUC_TSS_TwoStage")
...
# AUC (Wilcoxon rank-sum statistic) and max TSS
# obs_vals : favourability at presence points
# bg_vals  : favourability at background (pseudo-absence) points
# Returns a named list: auc, tss
compute_auc_tss <- function(obs_vals, bg_vals) {
  obs_vals <- obs_vals[is.finite(obs_vals)]
  bg_vals  <- bg_vals[is.finite(bg_vals)]
  n_pres   <- length(obs_vals)
  n_abs    <- length(bg_vals)

  if (n_pres < 2L || n_abs < 2L) return(list(auc = NA_real_, tss = NA_real_))

  # AUC via Wilcoxon statistic (exact, no ties correction needed for our use)
  auc_val <- tryCatch({
    wt <- wilcox.test(obs_vals, bg_vals, alternative = "greater", exact = FALSE)
    wt$statistic / (n_pres * n_abs)
  }, error = function(e) NA_real_)

  # max TSS: sweep candidate thresholds = unique sorted predicted values
  tss_val <- tryCatch({
    all_preds   <- c(obs_vals, bg_vals)
  ...
  list(auc = as.numeric(auc_val), tss = as.numeric(tss_val))
}
...
for (key in accepted_taxonkeys) {
  ...
  run_global_fold <- function(fold_i, train_sp, prev_tr, fold_label,
    ...
    auc_tss_full_test  <- compute_auc_tss(full_res_test$obs_vals,  full_res_test$fit_bg)
    auc_tss_full_train <- compute_auc_tss(full_res_train$obs_vals, full_res_train$fit_bg)
    ...
    boyce_full <- data.frame(
      ...
      auc_train   = auc_tss_full_train$auc,
      auc_test    = auc_tss_full_test$auc,
      tss_train   = auc_tss_full_train$tss,
      tss_test    = auc_tss_full_test$tss,
    ...
    if (has_eu_occ && !is.null(bg_eu_df) && !is.null(occ_eu_sp)) {
      ...
      boyce_eu <- data.frame(
        ...
        auc_train   = compute_auc_tss(eu_res_train$obs_vals, eu_res_train$fit_bg)$auc,
        auc_test    = compute_auc_tss(eu_res_test$obs_vals,  eu_res_test$fit_bg)$auc,
        tss_train   = compute_auc_tss(eu_res_train$obs_vals, eu_res_train$fit_bg)$tss,
        tss_test    = compute_auc_tss(eu_res_test$obs_vals,  eu_res_test$fit_bg)$tss,
  ...
  if (nrow(eu_occ_hab) < 20) {
    ...
    boyce_h_rows <- list()
      ...
      auc_tss_train <- compute_auc_tss(res_train$obs_vals, res_train$fit_bg)
      auc_tss_test  <- compute_auc_tss(res_test$obs_vals,  res_test$fit_bg)
      ...
      row_h <- data.frame(
        ...
        auc_train   = auc_tss_train$auc,
        auc_test    = auc_tss_test$auc,
        tss_train   = auc_tss_train$tss,
        tss_test    = auc_tss_test$tss,
    ...
    for (gi in seq_along(global_eu_bg_list)) {
      ...
      for (hi in seq_along(hab_bg_list)) {
        ...
        at_comb  <- compute_auc_tss(comb_obs, comb_bg)
        ...
        boyce_comb_rows[[comb_idx]] <- data.frame(
          ...
          auc_combined     = at_comb$auc,
          tss_combined     = at_comb$tss,
          n_eu_occ         = length(comb_obs),
          species          = species,
          taxonkey         = taxonkey,
          stringsAsFactors = FALSE
        )
        message(sprintf("    Combo %d/%d  (G-fold %d × H-fold %d):  Boyce = %s  AUC = %s  TSS = %s",
                        comb_idx, n_combos, g_fold_label, h_fold_label,
                        ifelse(is.finite(bc_comb),       round(bc_comb,       4), "NA"),
                        ifelse(is.finite(at_comb$auc),   round(at_comb$auc,   4), "NA"),
                        ifelse(is.finite(at_comb$tss),   round(at_comb$tss,   4), "NA")))
        gc()
      }
    }
    ...
    boyce_combined_summary <- if (nrow(boyce_combined_df) > 0 &&
      ...
      boyce_combined_df %>%
        ...
        dplyr::summarise(
          ...
          auc_combined_mean   = mean(auc_combined,   na.rm = TRUE),
          auc_combined_sd     = sd(auc_combined,     na.rm = TRUE),
          tss_combined_mean   = mean(tss_combined,   na.rm = TRUE),
          tss_combined_sd     = sd(tss_combined,     na.rm = TRUE),
          .groups = "drop"
        )
    } else {
      ...
      data.frame(species             = species,
                 ...
                 auc_combined_mean   = NA_real_,
                 auc_combined_sd     = NA_real_,
                 tss_combined_mean   = NA_real_,
                 tss_combined_sd     = NA_real_,
                 stringsAsFactors    = FALSE)
    }
  ...
  rm(list = setdiff(ls(), c(
    ...
    "get_boyce_cor",
    "compute_boyce_robust", "compute_auc_tss", "favourability_from_prob",
...
summarise_folds <- function(df, group_vars = c("species", "taxonkey", "area")) {
  df %>%
    ...
    dplyr::summarise(
      ...
      auc_test_mean    = mean(auc_test,    na.rm = TRUE),
      auc_test_sd      = sd(auc_test,      na.rm = TRUE),
      tss_train_mean   = mean(tss_train,   na.rm = TRUE),
      tss_train_sd     = sd(tss_train,     na.rm = TRUE),
      tss_test_mean    = mean(tss_test,    na.rm = TRUE),
      tss_test_sd      = sd(tss_test,      na.rm = TRUE),
      .groups = "drop"
    )
}
